...
class FeedPostCell: UITableViewCell {
    ...
    func configure(with info: FeedPostItemInfo ) {
        ...
        likesLabel.text = "Нравится: \(info.numberOrLikes)"
    ...
    private enum UIConstants {
        ...
        static let commentToLikesOffset: CGFloat = 14
    }
    ...
    private let commentButton: UIButton = {
        let button = UIButton(type: .system)
        button.tintColor = .black
        button.setImage(UIImage(systemName: "message"), for: .normal) //bubble.right
        return button
    }()
...
private extension FeedPostCell {
    func initialize() {
        ...
        commentLabel.snp.makeConstraints { make in
            ...
            make.bottom.equalToSuperview().inset(UIConstants.commentLabelFontSize)
        }
    }
    ...
    func configureCommentLabel(with comment: CommentShortInfo) {
        ...
        attributedString.addAttribute(NSAttributedString.Key.font,
                                       value: UIFont.boldSystemFont(ofSize: UIConstants.commentLabelFontSize),
                                       range: range)
